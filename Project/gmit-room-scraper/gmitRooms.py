...
out = csv.writer(open("myfile2.csv","w"), delimiter=',',quoting=csv.QUOTE_ALL)

i=0
row = [rooms,1]
headerRow = ["id","room", "capacity","campus"]
out.writerow(headerRow)
capacity = 22
campus = "Galway"
for room in rooms:
    capacity = room.rsplit(' (', 1)[1][:-1]
    room = room.rsplit(' (')[0]
    campus = room[:4]

    if campus[:1]=='G':
        campus = 'Galway'
        print("Campus in galway")
    elif campus == "LETT":
        campus = 'Letterfrack'
    row = [i, room , capacity,campus]
    out.writerow(row)
    i += 1
